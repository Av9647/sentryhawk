-- Run this CTAS script after Production table Backfill to derive exposure index ranges

-- Drop existing thresholds table if it exists
-- DROP TABLE IF EXISTS cve_db.exposure_index_thresholds;

-- Create a new thresholds table with all stats, cut-points, bucket counts, and human-readable ranges
CREATE TABLE cve_db.exposure_index_thresholds
WITH (
  format = 'PARQUET'
)
AS
WITH
  ...
  all_data AS (
    SELECT 'daily_global'   AS level, exposure_index FROM cve_db.cve_production_daily_global
    UNION ALL
    SELECT 'daily_vendor'   AS level, exposure_index FROM cve_db.cve_production_daily_vendor
    UNION ALL
    SELECT 'daily_product'  AS level, exposure_index FROM cve_db.cve_production_daily_product
    UNION ALL
    SELECT 'monthly_global' AS level, exposure_index FROM cve_db.cve_production_monthly_global
    UNION ALL
    SELECT 'monthly_vendor' AS level, exposure_index FROM cve_db.cve_production_monthly_vendor
    UNION ALL
  ...
  stats AS (
    SELECT
      ...
      MIN(exposure_index)                                      AS min_val,
      MAX(exposure_index)                                      AS max_val,
      approx_percentile(exposure_index, array[0.25,0.50,0.75]) AS pct_arr
    FROM all_data
    GROUP BY level
  ),
  ...
  bucketed AS (
    SELECT
      a.level,
      SUM(CASE WHEN exposure_index = 0                                        THEN 1 ELSE 0 END) AS cnt_none,
      SUM(CASE WHEN exposure_index > 0   AND exposure_index <= t.low_threshold  THEN 1 ELSE 0 END) AS cnt_low,
      SUM(CASE WHEN exposure_index > t.low_threshold
               AND exposure_index <= t.med_threshold                         THEN 1 ELSE 0 END) AS cnt_moderate,
      SUM(CASE WHEN exposure_index > t.med_threshold
               AND exposure_index <= t.high_threshold                        THEN 1 ELSE 0 END) AS cnt_high,
      SUM(CASE WHEN exposure_index > t.high_threshold                       THEN 1 ELSE 0 END) AS cnt_critical
    FROM all_data a
    JOIN thresholds t
      ON a.level = t.level
    GROUP BY a.level
...
-- SELECT * FROM cve_db.exposure_index_thresholds ORDER BY level;